...
namespace Shop_System.Controllers
{
    public class ExpensesController : ApiBaseController
    {
        ...
        [HttpGet("total")]
        public async Task<IActionResult> CalculateTotalExpenses([FromQuery] ExpenseCategory category)
        {
            var totalExpenses = await _expenseService.CalculateTotalExpensesAsync(category);
            return Ok(new ContentContainer<decimal>(totalExpenses, $"Total expenses for {category} calculated successfully."));
        }
    }
